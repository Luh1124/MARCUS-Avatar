...
_LANDMARK_DIR = Path(__file__).resolve().parent
BBRegressorParam = loadmat(_LANDMARK_DIR / "BBRegressorParam_r.mat")
mean_face = np.loadtxt(_LANDMARK_DIR / "test_mean_face.txt")
...
def load_lm_graph(graph_filename):
    # Configure TensorFlow GPU memory usage to avoid occupying all memory
    config = tf.compat.v1.ConfigProto()
    config.gpu_options.allow_growth = True  # Allocate GPU memory on demand instead of all at once
    # Optional: limit GPU memory fraction, for example 50%
    ...
    with tf.Graph().as_default() as graph:
        ...
        # Use the configured Session to limit GPU memory usage
        lm_sess = tf.compat.v1.Session(graph=graph, config=config)
