...
try:
    ...
    NEAREST_METHOD = Resampling.NEAREST
except ImportError:
    from PIL.Image import BICUBIC, NEAREST
    RESAMPLING_METHOD = BICUBIC
    NEAREST_METHOD = NEAREST
...
try:
    _NPVisibleDeprecation = np.VisibleDeprecationWarning
except AttributeError:
    from numpy.exceptions import VisibleDeprecationWarning as _NPVisibleDeprecation

warnings.filterwarnings("ignore", category=_NPVisibleDeprecation)
...
# resize and crop images for face reconstruction
def resize_n_crop_img_with_seg(img, lm, t, s, target_size=224., mask=None, seg=None):
    w0, h0 = img.size
    w = (w0*s).astype(np.int32)
    h = (h0*s).astype(np.int32)
    left = (w/2 - target_size/2 + float((t[0] - w0/2)*s)).astype(np.int32)
    ...
    if seg is not None:
        # ✅ Use PIL Image consistently for processing
        if isinstance(seg, np.ndarray):
            seg_pil = Image.fromarray(seg.astype(np.uint8))
        else:
            seg_pil = seg
        
        seg_pil = seg_pil.resize((w, h), resample=NEAREST_METHOD)
        seg_pil = seg_pil.crop((left, up, right, below))
        
        # Convert back to NumPy array if needed
        seg = np.array(seg_pil) if isinstance(seg, np.ndarray) else seg_pil
...
# utils for face reconstruction
def align_img_with_seg(img, lm, lm3D, mask=None, seg=None, target_size=224., rescale_factor=102.):
    """
    Return:
        transparams        --numpy.array  (raw_W, raw_H, scale, tx, ty)
        img_new            --PIL.Image  (target_size, target_size, 3)
        lm_new             --numpy.array  (68, 2), y direction is opposite to v direction
        mask_new           --PIL.Image  (target_size, target_size)
        seg_new            --PIL.Image  (target_size, target_size)
    Parameters:
        img                --PIL.Image  (raw_H, raw_W, 3)
        lm                 --numpy.array  (68, 2), y direction is opposite to v direction
        lm3D               --numpy.array  (5, 3)
        mask               --PIL.Image  (raw_H, raw_W, 3)
        seg                --numpy.array  (raw_H, raw_W)
    """
    ...
    # processing the image
    img_new, lm_new, mask_new, seg_new = resize_n_crop_img_with_seg(img, lm, t, s, target_size=target_size, mask=mask, seg=seg)
    # trans_params = np.array([w0, h0, s, t[0], t[1]])
    trans_params = np.array([w0, h0, s, t[0][0], t[1][0]])


    return trans_params, img_new, lm_new, mask_new, seg_new

# utils for face reconstruction
def align_img_3dffav3(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    """
    Return:
        transparams        --numpy.array  (raw_W, raw_H, scale, tx, ty)
        img_new            --PIL.Image  (target_size, target_size, 3)
...
def load_lm3d():
    ...
    from runtime_paths import topo_path
    Lm3D = loadmat(topo_path('similarity_Lm3D_all.mat'))
    Lm3D = Lm3D['lm']
