...
from .preprocess_sub import load_lm3d, align_img_3dffav3
...
class retinaface:
    ...
    def detector(self, im):
        ...
        if len(results_all)>0:
            ...
            trans_params, im, lm, _ = align_img_3dffav3(im, landmarks, self.lm3d_std)

            im = torch.tensor(np.array(im)/255., dtype=torch.float32).permute(2, 0, 1).unsqueeze(0)
            return trans_params, im, lm
    ...
    def __init__(self, args):
        if args.iscrop:
            if args.detector == 'mtcnn':
                ...
                r = retinaface(args.model_path, args.device)
                self.detector = r.detector
                self.detector_5p = r.detector_5p
                self.detector_106p = r.detector_106p
