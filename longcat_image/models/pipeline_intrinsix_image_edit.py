...
class IntrinsiXEditPipeline(DiffusionPipeline, FromSingleFileMixin):
    r"""
    The IntrinsiX-Edit pipeline for image editing.
    """
    ...
    def __call__(
        ...
        prompt_albedo: Union[str, List[str]] = None,
        prompt_normal: Union[str, List[str]] = None,
        prompt_rsd: Union[str, List[str]] = None,
        negative_prompt_albedo: Union[str, List[str]] = None,
        negative_prompt_normal: Union[str, List[str]] = None,
        negative_prompt_rsd: Union[str, List[str]] = None,   
        ...
        negative_prompt_albedo = "" if negative_prompt_albedo is None else negative_prompt_albedo
        negative_prompt_normal = "" if negative_prompt_normal is None else negative_prompt_normal
        negative_prompt_rsd = "" if negative_prompt_rsd is None else negative_prompt_rsd

        (prompt_embeds_albedo, text_ids) = self.encode_prompt(
            prompt=prompt_albedo, image=prompt_image, prompt_embeds=prompt_embeds, num_images_per_prompt=num_images_per_prompt
        )
        (prompt_embeds_normal, text_ids) = self.encode_prompt(
            prompt=prompt_normal, image=prompt_image, prompt_embeds=prompt_embeds, num_images_per_prompt=num_images_per_prompt
        )
        (prompt_embeds_rsd, text_ids) = self.encode_prompt(
            prompt=prompt_rsd, image=prompt_image, prompt_embeds=prompt_embeds, num_images_per_prompt=num_images_per_prompt
        )
        
        
        if self.do_classifier_free_guidance:
            (negative_prompt_embeds_albedo, negative_text_ids) = self.encode_prompt(
                prompt=negative_prompt_albedo,
                image=prompt_image,
                prompt_embeds=None, 
                num_images_per_prompt=num_images_per_prompt,
            )
            (negative_prompt_embeds_normal, negative_text_ids) = self.encode_prompt(
                prompt=negative_prompt_normal,
                image=prompt_image,
                prompt_embeds=None,
                num_images_per_prompt=num_images_per_prompt,
            )
            (negative_prompt_embeds_rsd, negative_text_ids) = self.encode_prompt(
                prompt=negative_prompt_rsd,
                image=prompt_image,
                prompt_embeds=None,
                num_images_per_prompt=num_images_per_prompt,
            )

        prompt_embeds = torch.cat([prompt_embeds_albedo, prompt_embeds_normal, prompt_embeds_rsd], dim=0)
        negative_prompt_embeds = torch.cat([negative_prompt_embeds_albedo, negative_prompt_embeds_normal, negative_prompt_embeds_rsd], dim=0)
        ...
        latents, image_latents, latents_ids, image_latents_ids = self.prepare_latents(
            ...
            calculated_height,
            calculated_width,
            prompt_embeds_albedo.dtype,
            prompt_embeds_albedo.shape[1],
            device,
            generator,
            latents,
        )
