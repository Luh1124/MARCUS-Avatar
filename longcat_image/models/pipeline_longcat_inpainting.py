...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> import torch
        >>> from pipeline_longcat_inpainting import LongCatImageInpaintingPipeline
        ...
        >>> ref_image = Image.open("ref.jpg").convert("RGB")
        >>> image = pipe(
        ...     input_image,
        ...     ref_image,
        ...     prompt,
        ...
        ... ).images[0]
        >>> image.save("longcat_image_inpainting.png")
        ```
"""
...
class LongCatImageInpaintingPipeline(DiffusionPipeline, FromSingleFileMixin):
    r"""
    The LongCat-Image-Inpainting pipeline for image inpainting.
    """
    ...
    def __init__(
        ...
        self.prompt_template_encode_prefix = "<|im_start|>system\nAs an image editing expert, first analyze the content and attributes of the input image(s). Do not describe or mention any accessories such as glasses. Then, based on the user's editing instructions, clearly and precisely determine how to modify the given image(s), ensuring that only the specified parts are altered and all other aspects remain consistent with the original(s).<|im_end|>\n<|im_start|>user\n<|vision_start|><|image_pad|><|vision_end|>"
        self.prompt_inpainting_template_encode_prefix = "<|im_start|>system\nAs an expert in 3D texture restoration and neural inpainting, analyze the provided reference render image to understand the subject's facial features, skin details, and the specific lighting environment. Do not describe or mention any accessories such as glasses. Based on this analysis and the user's text instruction, generate a semantic guide to seamlessly restore the missing or corrupted regions of the UV texture map. Ensure the completed texture strictly maintains consistency with the reference render's illumination, shading, and biological attributes, eliminating any seams or artifacts.<|im_end|>\n<|im_start|>user\n<|vision_start|><|image_pad|><|vision_end|>"
        self.prompt_template_encode_suffix = "<|im_end|>\n<|im_start|>assistant\n"
        self.default_sample_size = 128
        self.tokenizer_max_length = 512

    def _encode_prompt(self, prompt, image, use_input_image_for_inpainting=False):
        ...
        text = self.prompt_template_encode_prefix if not use_input_image_for_inpainting else self.prompt_inpainting_template_encode_prefix
    ...
    def encode_prompt(
        ...
        use_input_image_for_inpainting=False,
    ):
        prompt = [prompt] if isinstance(prompt, str) else prompt
        batch_size = len(prompt)
        # If prompt_embeds is provided and prompt is None, skip encoding
        if prompt_embeds is None:
            prompt_embeds = self._encode_prompt(prompt, image, use_input_image_for_inpainting)
    ...
    def __call__(
        ...
        if ref_image is not None:
            ref_image_size = ref_image[0].size if isinstance(ref_image, list) else ref_image.size
            ref_calculated_width, ref_calculated_height = calculate_dimensions(1024 * 1024, ref_image_size[0] * 1.0 / ref_image_size[1])
        ...
        if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
            ...
            if ref_image is not None:
                ref_image = self.image_processor.resize(ref_image, ref_calculated_height, ref_calculated_width)
                prompt_image = self.image_processor.resize(ref_image, calculated_height // 2, calculated_width // 2)
                use_input_image_for_inpainting = True
            else:
        ...
        (prompt_embeds, text_ids) = self.encode_prompt(
            prompt=prompt, image=prompt_image, prompt_embeds=prompt_embeds, num_images_per_prompt=num_images_per_prompt, use_input_image_for_inpainting=use_input_image_for_inpainting
        )
        if self.do_classifier_free_guidance:
            (negative_prompt_embeds, negative_text_ids) = self.encode_prompt(
                ...
                use_input_image_for_inpainting=use_input_image_for_inpainting
            )
