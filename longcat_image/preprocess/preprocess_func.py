...
def resize_crop_img(ori_img, trans_params):
    ...
    tar_img = ori_img.resize((w, h), resample=Image.Resampling.BICUBIC)
    tar_img = tar_img.crop((left, up, right, below))

    return tar_img


def resize_crop_img_retain_hr(ori_img, trans_params, output_size=None):
    ''' Align the image (resize + crop), but retain original high-resolution (HR)

    Args:
        ori_img: PIL.Image. The original input image.
        trans_params: numpy.array, (6). Contains w0, h0, s, t0, t1, target_size.
        output_size: int or None. The target output size. If None, output to original size (w0 x h0).
                     If specified (e.g., 1024), output to (output_size x output_size).
    Returns:
    ...
    # Use output_size if specified; otherwise use the original image size
    if output_size is not None:
        sf = float(output_size) / target_size
    ...
    right, below = left + final_size, up + final_size

    hr_img = ori_img.resize((w, h), resample=Image.Resampling.BICUBIC)
