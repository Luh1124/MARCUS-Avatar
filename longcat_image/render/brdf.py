...
def sqrt_(x: torch.Tensor, eps=1e-6) -> torch.Tensor:
    """
...
def get_phi(v: torch.Tensor):
    ...
    sin_theta = torch.clamp(sqrt_(1 - cos_theta * cos_theta), min=1e-6)
...
def sample_disney_specular(sample: torch.Tensor, roughness: torch.Tensor, wi: torch.Tensor):
    ...
    print(half)
    wo = F.normalize(reflect(wi.expand_as(half), half), dim=2, eps=1e-6)
    return wo
...
def smithG_GGX(ndv, a):
    a = a * a
    b = ndv * ndv
    return 1.0 / (ndv + sqrt_(a + b - a * b) + 1e-6)
...
def pdf_disney(roughness: torch.Tensor, metallic: torch.Tensor, wi: torch.Tensor, wo: torch.Tensor):
    ...
    half = F.normalize(wi + wo, dim=2, eps=1e-6)
    cosTheta = torch.abs(half[:, :, 2, ...])
    pdfGTR2 = GTR2(cosTheta, specularAlpha) * cosTheta
    pdfSpec = pdfGTR2 / torch.clamp(4.0 * torch.abs(torch.sum(wo * half, dim=2)), min=1e-6)
...
def Smith_G2_GGX(alphaSquared, NdotL, NdotV, eps=1e-6):
    NdotL = torch.clamp(NdotL, min=eps)
    NdotV = torch.clamp(NdotV, min=eps)
    a = NdotV * sqrt_(alphaSquared + NdotL * (NdotL - alphaSquared * NdotL))
    b = NdotL * sqrt_(alphaSquared + NdotV * (NdotV - alphaSquared * NdotV))
    return 0.5 / (a + b + eps)


def GGX_D(alphaSquared, NdotH, eps=1e-6):
    b = (alphaSquared - 1) * NdotH * NdotH + 1
    return alphaSquared / (np.pi * b * b + eps)
...
def pdf_ggx(color: torch.Tensor, roughness: torch.Tensor, metalness: torch.Tensor, wi: torch.Tensor, wo: torch.Tensor):
    ...
    NdotV = wi[:, :, 2, ...]
    h = F.normalize(wi + wo, dim=2, eps=1e-6)
    NdotH = h[:, :, 2, ...]
...
def eval_ggx(color: torch.Tensor, roughness: torch.Tensor, metalness: torch.Tensor, wi: torch.Tensor, wo: torch.Tensor, f0_override: torch.Tensor = None):
    """
    ...
    H = F.normalize(wi + wo, dim=2, eps=1e-6)
    NDotH = H[:, :, 2:3, ...]
    LDotH = torch.clamp(torch.sum(wo * H, dim=2, keepdim=True), 0.0, 1.0)
    roughness = roughness.unsqueeze(1)
    alpha = roughness * roughness
    alpha2 = alpha * alpha
    ...
    if f0_override is not None:
        # f0_override: (B, C, H, W), need to match wi/wo shape (B, K, C, H, W)
        specularF0 = f0_override.unsqueeze(1)  # (B, 1, C, H, W)
        diffuseReflectance = color
    else:
        specularF0 = baseColorToSpecularF0(color, metalness)  # (B, C, H, W)
        specularF0 = specularF0.unsqueeze(1)  # (B, 1, C, H, W)
        diffuseReflectance = color * (1 - metalness)
        
    # specularF0 is already (B, 1, C, H, W), don't unsqueeze again!
    f = evalFresnel(specularF0, shadowedF90(specularF0), LDotH)
    spec = f * G2 * D

    valid_mask = (NDotL > 1e-6).float()
    spec = spec * valid_mask
    diff = diffuseReflectance.unsqueeze(1) / np.pi
    mask = valid_mask
    return diff, spec, mask
