...
# NumPy 2.x: VisibleDeprecationWarning lives in numpy.exceptions, not np.*
try:
...
# resize and crop images
def resize_n_crop_img_mediapipe(img, lm, lm468, t, s, target_size=224., skin_mask=None, parse_mask=None):
    w0, h0 = img.size
    w = (w0 * s).astype(np.int32)
    h = (h0 * s).astype(np.int32)
    left = (w / 2 - target_size / 2 + float((t[0] - w0 / 2) * s)).astype(np.int32)
    ...
    lm468 = np.stack([lm468[:, 0] - t[0] + w0 / 2, lm468[:, 1] - t[1] + h0 / 2], axis=1) * s
    lm468 = lm468 - np.reshape(np.array([(w / 2 - target_size / 2), (h / 2 - target_size / 2)]), [1, 2])

    return img, lm, lm468, skin_mask, parse_mask
...
def align_img_mediapipe(img, lm, lm3D, lm468, skin_mask=None, parse_mask=None, target_size=224., rescale_factor=102.):
    """
    Return:
        transparams        --numpy.array  (raw_W, raw_H, scale, tx, ty)
        img_new            --PIL.Image  (target_size, target_size, 3)
        lm_new             --numpy.array  (68, 2), y direction is opposite to v direction
        lm468_new          --numpy.array  (468, 2)
        skin_mask_new      --PIL.Image  (target_size, target_size, 3)
        parse_mask_new     --PIL.Image  (target_size, target_size, 3)
    ...
    # processing the image
    img_new, lm_new, lm468_new, skin_mask_new, parse_mask_new = resize_n_crop_img_mediapipe(img,
                                                                       lm, 
                                                                       lm468,
                                                                       t,
                                                                       s,
    ...
    return trans_params, img_new, lm_new, lm468_new, skin_mask_new, parse_mask_new
